...
while true; do
    nfiles=`find ${input_directory} -name "*.txt" | wc -l`
    if [ "${nfiles}" -gt "0" ]; then

        # Extract File Name
        input_path=`find ${input_directory} -name "*.txt" | head -1`
        ...
        echo "!! file_name_csv", ${file_name_csv}
        ...
        mv ${work_path_csv} ${output_directory}
